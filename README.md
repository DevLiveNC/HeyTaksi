...
Mobil uygulamalar responsive ve dokunmatik önceliklidir. **PWA manifest** ve **Capacitor** kabuğu yolcu/sürücü uygulamalarına eklenmiştir: tarayıcıdan ana ekrana ekleme, Android APK ve iOS native proje. Oturum native’de Preferences’a yazılır, konum eklentisi işletim sistemi iznini kullanır, girişte cihaz `ios`/`android` olarak kaydedilir. Push bildirimi, sürücü arka plan GPS’i ve mağaza yayını sonraki fazdadır. Adımlar için `docs/MOBILE.md`. Vercel test kurulumu için `docs/VERCEL.md` dosyasına bakın.
